...
[case03](../case03_carbonate/) with only the chemistry entries changed.

| File | Role |
|---|---|
| `test3_TxtOutputs.dat` | **golden trace**, 41 near-field rows × 13 cols |
| `testco2.csv`, `testdiffuser.csv`, `ambient.csv`, `effluent.csv`, `mixzone.csv` | inputs, carried over from case03 and verified against the `.dat` echo |
...
This folder was `case04_macoma_ta_dic` until 2026-09-09. The word came off because these exe runs are an earlier entry of Ebb's diffuser with known slips (2 m for 2 ft ports, a 35 psu / 10 °C effluent, 0.219 L/s, mixing zones typed in metres, a chemistry table that was not the site's) and the name read as the site; the site's actual values are the standalone Macoma case (`reference_cases/pending/macoma_*` until the exe has run it). Data unchanged.
