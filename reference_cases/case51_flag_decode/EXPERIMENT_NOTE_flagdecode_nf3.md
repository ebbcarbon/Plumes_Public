...
**Question.** Which GUI control does `.prj` nearfield_flags position 3 (1-indexed; python [2]) carry? Undecoded in PLAN section 6; constant in every archived project, flipped here and nowhere else.

## Run it

1. Open `flagdecode_nf3.prj` in the exe. **Do not open a hand-maintained
...
|---|---|
| near-field flag 3 (0 -> 1) | the shoreline checkbox is the natural candidate (0 in every archived project, and the box has never been saved ticked) |
| GUI on load | exactly one control differs from flagdecode_base, or none (then the flag is not a dialog control) |
| as-run flag value | 1 if the flag is a control the exe respects and saves; 0 if the exe rewrites it from session state on run (either way the bit's ownership is decided) |
