...
**Question.** The site's actual values on the chronic ambient (0.05 m/s): does the exe reproduce the port's near field, boundaries and pH at 2 ft spacing, 5900 L/h of intake water and the site's uniform chemistry -- the configuration the dose study is built on, which no archived run carries?

## Run it

1. Open `macoma_chronic.prj` in the exe. **Do not open a hand-maintained
...
2. Set these in the GUI, which the `.prj` cannot carry (the output interval is **already 5** in the file, and needs no typing):
   ...
   ⚠️ **The carbonate tab is GUI-only.** `AmbientChem_macoma_chronic.csv` is written beside the
...
| ambient current | 0.05 m/s at 90 deg |
...
|---|---|
| termination | `oscillation limit` at t = 143 s |
| near-field end: flux-averaged dilution | 240 |
| near-field end: distance from the diffuser | 7.64 m |
| trapping depth (centreline at the end) | 1.78 m |
| plume diameter at the end | 0.63 m |
| wastefield width handed to the far field | 15.3 m |
| merging | the jets merge in shallow overlap before the end (`merging happened` banner expected) |
| port pH (total) | 10.990 |
| pH at the end of the near field (total) | 7.783 |
| Omega_brucite falls back through 1 (flux-averaged; the exe cannot print it) | dilution 2.53, 0.15 s, 3.4 cm from the port |
| acute mixing-zone boundary (6.31 m): dilution / pH | 220 / 7.787 (nearfield) |
| chronic mixing-zone boundary (63.09 m): dilution / pH | 306 / 7.771 (farfield) |
| far field at its last row | 508 m, dilution 1887, width 165.6 m |
...
case       20e1080e54b7ff8801bd9b3e153ff0f8ed62b30855d6fa2c56452d2bd7b383f0
python     3.14.4
