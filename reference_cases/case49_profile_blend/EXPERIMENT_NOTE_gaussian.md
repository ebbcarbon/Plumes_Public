# profile_merged_gaussian

**Question.** How does the exe walk its `Gaussian Profile` from round to slab as neighbouring plumes merge -- and is it the same linear law in d/L the default parabola uses?

## Run it

1. Open `profile_merged_gaussian.prj` in the exe. **Do not open a hand-maintained
...
2. Set these in the GUI, which the `.prj` cannot carry (the output interval is **already 1** in the file, and needs no typing):
   ...
   - **Similarity profile: select `Gaussian Profile`** -- the selector opens on `Default Profile` every time (case48); check it before pressing run
...
| `Dilutn / CL-Dil` before the merge banner, developed rows (`Dilutn` > 5) | 3.6700, flat to five figures (case48 measured 3.66998 on the unmerged case03 geometry) |
| `Dilutn / CL-Dil` once `P-dia` >= 2 x 0.6 m (deep slab) | 2.1471 -- the profile's own slab integral (2 sqrt(k) / (sqrt(pi) erf(sqrt(k)))) |
| `Dilutn / CL-Dil` for 0.6 < `P-dia` < 1.2 m | **the assumption under test**: max(2.1471, 3.6700 + 1.5228 * (1 - d/L)), the parabola's linear walk applied between this profile's anchors. Alternatives to look for: no blend at all (flat at the round value), or a geometric blend that is not a straight line in d/L |
...
- Copy the trace aside as `profile_merged_gaussian.dat` before any further run.
...
case       d5f1e5b11a3598baba676e1f377a2b33fee6aec3854d016e4b06d76371a73494
python     3.14.4
