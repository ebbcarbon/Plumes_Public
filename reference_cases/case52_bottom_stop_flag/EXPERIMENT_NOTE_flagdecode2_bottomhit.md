...
**Question.** The bottom-hitting reference arm: does this geometry stop on seabed contact with everything as loaded?

## Run it

1. Open `flagdecode2_bottomhit.prj` in the exe. **Do not open a hand-maintained
...
2. Set these in the GUI, which the `.prj` cannot carry (the output interval is **already 10** in the file, and needs no typing):
   ...
   - stop plume at bottom hit: **leave as loaded** -- its on-load state is data; write it down before running
...
| port's own integration | the port integrates this case to `seabed` at t = 21 s, flux-averaged dilution 93 -- seabed contact is early and unambiguous (edge criterion `depth + radius >= 10.3 m`, the row 106 convention) |
| build fingerprint | record which exe was launched; the far-field width identifies the build regardless (row 263 vs row 96) |
