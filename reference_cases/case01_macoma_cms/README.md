...
|---|---|---|
| `Macoma2.prj` | 2026-02-19 10:36 | project file, 149 lines |
| `ModelResults_TxtOutputs.dat` | 2026-02-19 10:37 | **golden trace**, 84 step rows to step 420 |
| `macoma2ambient.csv` | 2025-12-02 | 6 depth levels, 0–15 m |
| `macoma2diffuser.csv` | 2025-12-02 | 1 active case |
| `macoma2effluent.csv` | 2025-12-04 | 8 flow cases, row 1 active |
| `macoma2mixzone.csv` | 2025-12-02 | acute 20.7 m, chronic 207 m |
...
1. **The per-table integer blocks are unit selectors, not case flags.** Counts
   ...
   The Macoma effluent block is `[1, 2, 1, 1, 1]` where the example is all `1` —
...
3. **The near-field settings block is 3 reals + 6 flags + interval + filename.**
   Example `[1,1,0,2,1,1]`, Macoma `[1,0,0,3,1,0]`. Position 4 is the max
...
4. **The far-field flag block is 7 flags + count.** Example `[1,0,0,1,1,1,0]`,
   Macoma `[1,0,0,1,0,0,0]`. Position 4 is presumably the eddy-diffusivity law
...
> ⚠️ **Superseded by [case02](../case02_macoma_mgd/README.md).** I originally read
