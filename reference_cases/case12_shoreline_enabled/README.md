...
|---|---|---|---|
| [case08](../case08_shoreline/) | 45° | (not set) | byte-identical to the no-shoreline run |
...
This folder was `case12_macoma_shoreline_enabled` until 2026-09-09. The word came off because these exe runs are an earlier entry of Ebb's diffuser with known slips (2 m for 2 ft ports, a 35 psu / 10 °C effluent, 0.219 L/s, mixing zones typed in metres, a chemistry table that was not the site's) and the name read as the site; the site's actual values are the standalone Macoma case (`reference_cases/pending/macoma_*` until the exe has run it). Data unchanged.
