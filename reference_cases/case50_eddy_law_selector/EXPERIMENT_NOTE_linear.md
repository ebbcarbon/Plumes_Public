...
**Question.** Which `.prj` flag encodes the far-field eddy-diffusivity selector, what does the header print for `Linearly varying eddy diffusivity`, and does the exe's linearly varying law match the port's?

## Run it

1. Open `eddy_law_linear.prj` in the exe. **Do not open a hand-maintained
...
2. Set these in the GUI, which the `.prj` cannot carry (the output interval is **already 1** in the file, and needs no typing):
   ...
   - **far-field eddy diffusivity: select `Linearly varying eddy diffusivity`** -- the selector opens on the 4/3 law; this run exists to see what selecting another option does
   ...
   - far-field eddy diffusivity: **Linearly varying eddy diffusivity**. The selector offers three laws; `4/3 power law` is the GUI default and the only one ever archived,
...
| far-field `Dilution` at 100 m | **900.5** under this law (the 4/3 default gives 997.8 on the same near field); the port's Brooks is confirmed to 5e-4 against the exe's own standalone calculator on the 4/3 law (row 119), so a match here also confirms the other law's implementation |
| far-field `Width` at 100 m | 176.14 m |
| far-field `Dilution` at 200 m | **1419.0** under this law (the 4/3 default gives 1790.9 on the same near field); the port's Brooks is confirmed to 5e-4 against the exe's own standalone calculator on the 4/3 law (row 119), so a match here also confirms the other law's implementation |
| far-field `Width` at 200 m | 307.46 m |
| far-field `Dilution` at 500 m | **2986.9** under this law (the 4/3 default gives 4964.5 on the same near field); the port's Brooks is confirmed to 5e-4 against the exe's own standalone calculator on the 4/3 law (row 119), so a match here also confirms the other law's implementation |
| far-field `Width` at 500 m | 701.44 m |
...
- Copy the trace aside as `eddy_law_linear.dat` and the rewritten project as `asrun_eddy_law_linear.prj` before any further run.
...
generated  2026-08-26T19:57:47Z
case       5f1a3abdeddd384d237d6ab2fdcf3fd5612af6804fd83be6167b4d117d560b53
python     3.14.4
